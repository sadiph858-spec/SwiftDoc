...
public class PDFFile {
    private long id; private String name,path,date; private long size; private boolean isFavorite; private long lastOpened;
    public PDFFile() {}
    public PDFFile(String name,String path,String date,long size){this.name=name;this.path=path;this.date=date;this.size=size;}
    public PDFFile(long id,String name,String path,String date,long size,boolean isFav,long last){
        this.id=id;this.name=name;this.path=path;this.date=date;this.size=size;this.isFavorite=isFav;this.lastOpened=last;}
    ...
    public void setLastOpened(long v){lastOpened=v;}
    public String getFormattedSize(){
        if(size<=0)return "0 B"; String[]u={"B","KB","MB","GB"}; int i=0; double s=size;
        while(s>=1024&&i<u.length-1){s/=1024;i++;} return String.format(Locale.getDefault(),"%.1f %s",s,u[i]);}
